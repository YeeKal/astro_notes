...
![claude](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude.png)
...
![claude_register.png](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_register.png)

## 体验

注册完就可以愉快地开启新的对话了。输入框右侧有两个按钮，其中一个是发送键，另一个回形针样的图标是添加附件的按钮，鼠标悬浮会看到提示，支持最多上传5个文件，每个最大10M。支持常见的pdf, csv, txt等文本文件。

![](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_chat.png)

首先看下**中文理解能力**, 回答有条不紊。

![](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_chinese.png)

用中文做一下**逻辑测试**，回答完全正确，同时还带有解析。但是解析的第二步和第三步有矛盾，所以这道题看上去更像网上出题人太多了统计出来的结果，而不是推理出来的结果。

![claude_logic](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_logic.png)

以上的测试在前两年还可以看作语言模型智能的体现，但是在今天已经是大模型的标配了。这些测试在实际生活只能略做消遣，接下来进行一些工作生活中可能会用到的能力——**阅读文本**。

有一篇关于医药研究的文章，不想完整的读完，扔给它，让它总结一下：

![claude_news](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_news.png)

看英文有些吃力，让它给个中文版的摘要：

![claude_news](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_news_chinese.png)
...
![claude_n2](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_n2.png)

小小的提示了一下，主题虽然对了，但是其中的内容却依然是胡编乱造的。
![claude_n3](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_n3.png)
...
![claude_n4](https://cdn.jsdelivr.net/gh/YeeKal/Doc/blog/imgs/claude_n4.png)
